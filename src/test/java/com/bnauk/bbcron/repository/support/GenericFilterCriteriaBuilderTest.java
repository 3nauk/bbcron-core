...
class GenericFilterCriteriaBuilderTest {
  ...
  @Test
  void addCondition() {
    andConditions.add(FilterCondition.builder().field("name").value("JohnDoe")
        .operator(FilterOperationEnum.EQUAL).build());
    orConditions.add(FilterCondition.builder().field("age").value("34")
        .operator(FilterOperationEnum.GREATER_THAN).build());
    assertNotNull(genericFilterCriteriaBuilder.addCondition(andConditions, orConditions));
    assertTrue(genericFilterCriteriaBuilder.addCondition(andConditions, orConditions).getClass()
        .equals(Query.class));
  ...
  void addCondition_with_or_criteria_empty() {
    andConditions.add(FilterCondition.builder().field("age").value("34")
        .operator(FilterOperationEnum.GREATER_THAN).build());
    assertNotNull(genericFilterCriteriaBuilder.addCondition(andConditions, orConditions));
    assertTrue(genericFilterCriteriaBuilder.addCondition(andConditions, orConditions).getClass()
        .equals(Query.class));
  ...
  void addCondition_with_invalid_function() {
    andConditions.add(FilterCondition.builder().field("name").value("JohnDoe")
        .operator(FilterOperationEnum.EQUAL).build());
    andConditions.add(FilterCondition.builder().field("age").value("34")
        .operator(FilterOperationEnum.IS).build());
